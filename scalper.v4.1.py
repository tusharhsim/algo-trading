...
## variable data ##
#instrument = ['BANKNIFTY22505', 1200]
instrument = ['NIFTY22505', 1800]
token_data = {}
placed_orders = {}
open_positions = {}
order_adjustment = {}
...
async def margin_updater():
        ...
        try:
                ...
                if len(enctokens) > 0:
                    ...
                    print('\ntotal fund available\t%s\n' %int(sum([i['margin'] for i in enctokens.values()])))
...
async def Trade(contract):
        try:
                ...
                print(f'default sl  - {sl}')
                print(f'default tgt - {tgt}')
                risk = []
                reward = []
                for i in range(1,6):
                        risk.append(round(buying_price*(1-(i*5)/100),2))
                        reward.append(round(buying_price*(1+(i*5)/100),2))
                print(f'rewards in order \t{reward}')
                print(f'risk in order \t\t{risk}')
...
driver.find_element(By.ID, "userid").send_keys('TC7945')
driver.find_element(By.ID, "password").send_keys('jaimatadi')
driver.find_element(By.TAG_NAME, 'button').click()
driver.find_element(By.ID, 'pin').send_keys('161289')
driver.find_element(By.TAG_NAME, 'button').click()
